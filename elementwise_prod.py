...
a_cpu = np.random.randn(MATRIX_SIZE_h, MATRIX_SIZE_w).astype(np.float32)
b_cpu = np.random.randn(MATRIX_SIZE_h, MATRIX_SIZE_w).astype(np.float32)
...
c_gpu = gpuarray.empty((MATRIX_SIZE_h, MATRIX_SIZE_w), np.float32)

# get the kernel code from the template 
# by specifying the constants MATRIX_SIZE and BLOCK_SIZE
kernel_code = kernel_code_template % { 
    'MATRIX_SIZE_h': MATRIX_SIZE_h,
    'MATRIX_SIZE_w': MATRIX_SIZE_w,
...
end_gpu=time.time()
print "Time taken GPU:",np.round((end_gpu-start_gpu),4)

start_cpu=time.time()
# compute reference on the CPU to verify GPU computation
c_cpu = np.dot(a_cpu, b_cpu)
end_cpu=time.time()
print "Time taken CPU:",np.round((end_cpu-start_cpu),4)
